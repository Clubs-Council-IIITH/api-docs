...
if [ -d "./services" ]; then
    read -p 'services was found in the required location, do you want to reclone? [y/N]' input
    case ${input:0:1} in
	y|Y)
	    sudo rm -rf services
...
if [ -d "./services" ]; then
    print_bold "Building docker images, this might take serveral minutes the first time.."
